...
export function Button({ children, mode = 'yellow', className = '', ...props }: Props) {
  ...
  const modeClasses: {[key in ButtonMode]: string} = {
    ...
    yellow: 'bg-first-color text-black hover:bg-black hover:text-white disabled:hover:bg-first-color disabled:hover:text-black',
    green: 'bg-third-color text-black hover:bg-black hover:text-white disabled:hover:bg-third-color disabled:hover:text-black',
  ...
  return (
    <button
      ...
      className={classes(
        modeClasses[mode],
        `font-sans text-lg font-bold flex items-center rounded-md cursor-pointer border border-solid border-black py-2 px-4 disabled:opacity-50 disabled:bg-bg-grey disabled:text-text-grey disabled:cursor-default transition-all`,
        className
      )}>
      {children}
    </button>
